...
def restore_exam_typography(text: str, is_option: bool = False) -> str:
    ...
    # 2.2 Correção de aglutinações e fragmentações de palavras comuns do OCR
    t = re.sub(r"\bsequ[êe]nciaos\b", "sequência dos", t, flags=re.IGNORECASE)

    # 2.3 Normalização de pontuação e espaçamentos (remove espaço antes de vírgula/ponto/dois-pontos/etc.)
    t = re.sub(r"[ \t]+([,.:;?!])", r"\1", t)
    ...
    # 2.4 Recomposição de ênclises e pronomes oblíquos descolados (ex: "impede - nos" -> "impede-nos", "origina -s e" -> "origina-se")
    ...
    # 2.5 Correções lexicais sistemáticas de ruídos do OCR em Português
    ocr_word_fixes = [
    ...
    # 2.6 Correção de numerais romanos colados ou corrompidos em enunciados
    ...
    if is_option:
        # 1. Remove prefixo residual de letra da alternativa ("a) ", "A. ", "a ", "? ", "| ")
        t = re.sub(r'^[ \t]*(?:(?:\([a-eA-E]\)|[a-eA-E]\s*[\)\.\-–—:])|[a-eA-E]\s+(?=[I|V|X\d\?\|\-–—])|[\?\|]\s+)\s*', '', t)
        # 2. Remove sufixo residual (". C", ". d", "- b")
        t = re.sub(r'\s*[\.\-–—:,]\s*[a-eA-E]\s*$', '', t)
        # 3. Normaliza numerais romanos em opções de múltipla escolha
        t = re.sub(r'(^|;\s*|\s+)(?:\||1|l|I)\s*-\s*', r'\1I - ', t)
        t = re.sub(r'(^|;\s*|\s+)(?:ll|11|Il|lI|II)\s*-\s*', r'\1II - ', t)
        t = re.sub(r'(^|;\s*|\s+)(?:lll|111|Ill|lIl|llI|III|I\s+ll|I\s+lI)\s*-\s*', r'\1III - ', t)
