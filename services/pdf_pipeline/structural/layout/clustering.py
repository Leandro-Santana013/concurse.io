...
def _deterministic_dbscan(
    ...
    *,
    ...
    """Small NumPy-only DBSCAN fallback for reproducible base installations."""
    if len(features) == 0:
    ...
    if use_robust_scaler:
        ...
        values = (features - center) / scale
    else:
        values = features
    distances = np.sqrt(
        ((values[:, None, :] - values[None, :, :]) ** 2).sum(axis=2)
    )
    neighbours = [np.flatnonzero(row <= eps).tolist() for row in distances]
    core = [len(items) >= min_samples for items in neighbours]
    labels = np.full(len(features), -1, dtype=int)
    visited = np.zeros(len(features), dtype=bool)
    cluster_id = 0
    for start in range(len(features)):
        if visited[start] or not core[start]:
            continue
        visited[start] = True
