...
class ColumnModeler:
    ...
    def detect(
        ...
        if GaussianMixture is None:
            return self._fallback_by_gaps(page, text_elements, gutter)
        training_elements = [
            element
    ...
    def _fallback_by_gaps(
        self,
        page: PageModel,
        elements: list[PhysicalElement],
        gutter: GutterEvidence,
    ) -> ColumnDetection:
        """Deterministic NumPy-only fallback when scikit-learn is absent."""
        training = [
            element
            for element in elements
            if element.bbox.width / max(float(page.width), 1e-9)
            <= self.config.max_training_width
        ]
        if len(training) < 4:
            return self._fallback(page, elements, bics={1: 0.0})

        ordered = sorted(
            training,
            key=lambda item: (
                (float(item.bbox.nx0) + float(item.bbox.nx1)) / 2.0,
                item.id,
            ),
        )
        centers = [
        ...
        threshold = max(
            0.08,
            float(self.config.min_center_separation),
            float(self.config.min_geometric_gap) * 2.0,
        )
        split_indexes = [
        ...
        )[: max(0, self.config.max_columns - 1)]
        split_indexes.sort()
        if not split_indexes:
            return self._fallback(page, elements, bics={1: 0.0})

        groups: list[list[PhysicalElement]] = []
        start = 0
        ...
        columns: list[ColumnModel] = []
        total = max(len(training), 1)
        for index, group in enumerate(groups):
            x0 = min(float(item.bbox.nx0) for item in group)
            x1 = max(float(item.bbox.nx1) for item in group)
            center = sum(
                (float(item.bbox.nx0) + float(item.bbox.nx1)) / 2.0
                for item in group
            ) / len(group)
            ...
            if index + 1 < len(groups):
                boundary = (x1 + float(groups[index + 1][0].bbox.nx0)) / 2.0
                gutter_score = self._gutter_score_at(gutter, boundary) or 1.0
            columns.append(
                ColumnModel(
                    index=index,
                    center_x_norm=center,
                    x0_norm=x0,
                    x1_norm=x1,
                    width_norm=max(0.0, x1 - x0),
                    weight=len(group) / total,
                    sample_count=len(group),
                    bic=-float(index + 1),
                    bic_gain=(
                        gaps[split_indexes[index]]
                        if index < len(split_indexes)
                        else 0.0
                    ),
                    gutter_score=gutter_score,
                    gutter_validated=gutter_score >= self.config.min_gutter_score,
                )
            )
        assignments, spanning = self._assign_all(page.elements, columns)
        return ColumnDetection(
            ...
            assignments=assignments,
            spanning_ids=spanning,
            bics={1: 0.0, len(columns): -float(len(columns))},
            selected_k=len(columns),
        )
