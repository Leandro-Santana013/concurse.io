...
def download_pdf_file(url: str, dest_path: str, timeout: int = 30) -> bool:
    """Faz o download de um arquivo PDF com headers de navegador e tratamento de redirecionamento."""
    headers = {
...
def process_exam_async(exam_id: int, gabarito_override: Optional[str] = None):
    """
    Worker completo de ingestão e processamento assíncrono:
    ...
    5. Pareamento e Persistência no Banco de Dados (85-100%)
    """
    with Session() as session:
        exam = session.query(Exam).filter_by(id=exam_id).first()
        if not exam:
    ...
    with Session() as session:
        ...
        # Salva no catálogo de busca para respostas instantâneas futuras
        if source_url:
